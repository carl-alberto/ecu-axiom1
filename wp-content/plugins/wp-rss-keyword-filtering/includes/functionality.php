...
function wprss_kf_check_post_item_keywords( $item, $feed_ID, $permalink ) {
    ...
    $postTagsArray = array_map('html_entity_decode', $postTagsArray);
 ...
 *
 * @param SimplePie_Category $tag The SimplePie tag object
 * @return string The label of the tag
 * @since 1.3
 */
function wprss_kf_get_tag_label( $tag ) {
    return WPRSS_MBString::mb_strtolower(html_entity_decode($tag->get_label()));
}
...
function wprss_kf_filter_item( $item, $keyword, $filtering = array() ) {
    ...
    foreach ( $filtering as $prop ) {
        switch( strtolower( $prop ) ) {
            case 'title':
                $found = !(WPRSS_MBString::mb_stripos(html_entity_decode($item->get_title()), $keyword) === FALSE );
                if ( $found === TRUE ) return TRUE;
                break;
            case 'content':
                $found = !(WPRSS_MBString::mb_stripos(html_entity_decode($item->get_content()), $keyword) === FALSE );
                if ( $found === TRUE ) return TRUE;
                break;
        }
    }
