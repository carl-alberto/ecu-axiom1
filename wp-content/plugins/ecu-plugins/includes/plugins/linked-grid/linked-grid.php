<?php
	...
	class Linked_Grid extends Abstract_Ecu_Shortcode {
		...
		public function initialize(){
			...
			if ( is_admin() ) {
				add_editor_style(plugins_url('/ecu-plugins/includes/plugins/tabbed-grid/css/style.css') );
			}
		...
		public function enqueue_scripts() {
			wp_register_style( 'ecu-shortcode-tabbed_grid', plugins_url('/ecu-plugins/includes/plugins/tabbed-grid/css/style.css') );
		}
			...
			if($linked_grid = get_field('ui_linked_grid', $attrs['linked_id'])){
				...
				foreach($linked_grid as $post){
					...
					if($post_bg || is_user_logged_in()){
						$links .= "<a href='{$post_link}' class='{$col_size}' target='{$target}'><div class='tab-container'><div class='grid_post $post_size' style='background-image:url({$post_bg});'>";
						if(!$post_bg){
