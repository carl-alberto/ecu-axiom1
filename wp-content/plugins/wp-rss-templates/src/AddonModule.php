...
use RebelCode\Wpra\Templates\Twig\ThumbnailsTwigExtension;
...
class AddonModule implements ModuleInterface
{
    ...
    {
        // Load text domain
        load_plugin_textdomain('wprss', false, $c->get('wpra_tmp/languages_path'));
    ...
    public function getFactories()
    {
        return [
            ...
            /*
             * The path to the addon's languages directory, relative to the plugins directory.
             *
             * @since 0.3.1
             */
            'wpra_tmp/languages_path' => function () {
                return dirname(plugin_basename($this->pluginFilePath)) . '/languages/';
            },
            /*
            ...
            /*
             * The URL to the dist directory where the addon's front-end files can be found. Trailing slash is included.
             *
             * @since 0.1
             */
            'wpra_tmp/dist_dir_url' => function (ContainerInterface $c) {
                return $c->get('wpra_tmp/plugin_dir_url') . 'dist/';
            },
            /*
            ...
            'wpra_tmp/js_dir_url' => function (ContainerInterface $c) {
                return $c->get('wpra_tmp/dist_dir_url') . 'js/';
            },
            /*
            ...
            'wpra_tmp/css_dir_url' => function (ContainerInterface $c) {
                return $c->get('wpra_tmp/dist_dir_url') . 'css/';
            },
            /*
             ...
             * 0.3
             */
            'wpra_tmp/twig/extensions/thumbnails' => function (ContainerInterface $c) {
                return new ThumbnailsTwigExtension();
            },
            /*
             * Alias for "wpra/feeds/templates/audio_features_enabled" which defaults to false if it doesn't exist.
             *
             * @since 0.3
             */
            'wpra_tmp/audio_features_enabled' => function (ContainerInterface $c) {
                return $c->has('wpra/feeds/templates/audio_features_enabled')
                    ? $c->get('wpra/feeds/templates/audio_features_enabled')
                    : false;
            },
            /*
            ...
            'wpra_tmp/templates/feeds/grid_parts_paths' => function (ContainerInterface $c) {
                $partsDir = implode(DIRECTORY_SEPARATOR, ['feeds', 'grid', 'parts']);

                $parts = [
                    'title' => $partsDir . DIRECTORY_SEPARATOR . 'title.twig',
                    'image' => $partsDir . DIRECTORY_SEPARATOR . 'image.twig',
                    'excerpt' => $partsDir . DIRECTORY_SEPARATOR . 'excerpt.twig',
                    'information' => $partsDir . DIRECTORY_SEPARATOR . 'information.twig',
                ];

                if ($c->get('wpra_tmp/audio_features_enabled')) {
                    $parts['audio'] = $partsDir . DIRECTORY_SEPARATOR . 'audio.twig';
                }
            ...
            'wpra_tmp/templates/feeds/grid_model_options' => function (ContainerInterface $c) {
                ...
                if ($c->get('wpra_tmp/audio_features_enabled')) {
                    $options['card_fields_order']['audio'] = 3;
                }
            ...
            'wpra_tmp/templates/feeds/templates_model_options_tooltips' => function (ContainerInterface $c) {
                return [
                    ...
                    'columns_number' => __('The maximum number of columns. The actual number of columns shown may be lower depending on the page width and screen size. Recommended values are between 2 and 5. ', 'wprss'),
                    'show_borders' => __('Check this box to enable a border around each item in the grid.', 'wprss'),
    ...
    public function getExtensions()
    {
        return [
            ...
            /*
             * Enables the audio features in templates.
             *
             * 0.3
             */
            'wpra/feeds/templates/audio_features_enabled' => function () {
                return function_exists('wpra_is_audio_file');
            },
            /*
            ...
            /*
             * Registers this addon's Twig extension.
             *
             * 0.3
             */
            'wpra/twig/extensions' => function (ContainerInterface $c, $prev) {
                $prev[] = $c->get('wpra_tmp/twig/extensions/thumbnails');
