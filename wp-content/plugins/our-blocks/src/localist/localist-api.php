...
class Localist_Block {
     ...
     */
    protected $url = 'https://calendar.ecu.edu/api/2/';
    ...
    public function get_events( $parameters ) {
        ...
        if( ! empty( $parameters['departments'] ) ) {
            if(!is_array($parameters['departments'])) {
                $parameters['departments'] = explode( ',' , $parameters['departments'] );
            }
            foreach( $parameters['departments'] as $filter ) {
                $query[] = 'type[]=' . absint( $filter );
            }
        }

        if( ! empty( $parameters['events'] ) ) {
            if(!is_array($parameters['events'])) {
                $parameters['events'] = explode( ',' , $parameters['events'] );
            }
            foreach( $parameters['events'] as $filter ) {
                $query[] = 'type[]=' . absint( $filter );
            }
        }

        if( ! empty( $parameters['audiences'] ) ) {
            if(!is_array($parameters['audiences'])) {
                $parameters['audiences'] = explode( ',' , $parameters['audiences'] );
            }
            foreach( $parameters['audiences'] as $filter ) {
                $query[] = 'type[]=' . absint( $filter );
            }
        }
        ...
        foreach ( $events as $i => $data ){
            $event = [
                'id'            =>  $data->event->id,
                'url'           =>  $data->event->localist_url,
                'title'         =>  esc_html( $data->event->title ),
                'location'      =>  !empty( $data->event->location ) ? esc_html( $data->event->location ) : false,
                'description'   =>  !empty( $data->event->description_text ) ? esc_html( $data->event->description_text ) : false,
                'hours'         =>  []
            ];

            if ( $data->event->event_instances[0]->event_instance->all_day ) {
                $event[ 'hours' ][ 'start' ] = [
                    'time'  => $data->event->event_instances[0]->event_instance->start,
                    'label' => 'All Day'
                ];
            } else {
                if( ! empty( $data->event->event_instances[0]->event_instance->start ) ) {
                    $event[ 'hours' ][ 'start' ] = [
                        'time'  => $data->event->event_instances[0]->event_instance->start,
                        'label' => date( 'g:i a', strtotime( substr( $data->event->event_instances[0]->event_instance->start, 0, -6 ) ) )
                    ];

                    if ( ! empty( $data->event->event_instances[0]->event_instance->end ) ) {
                        $event[ 'hours' ][ 'end' ] = [
                            'time'  => $data->event->event_instances[0]->event_instance->end,
                            'label' => ' to ' . date( 'g:i a', strtotime( substr($data->event->event_instances[0]->event_instance->end, 0, -6) ) )
                        ];
                    } else {
                        $event[ 'hours' ][ 'end' ] = false;
                    }
                }
            }

            $date = strtotime( substr( $data->event->event_instances[0]->event_instance->start, 0, -6 ) );
            $event[ 'monthabbr' ] = date( 'M', $date );
            $event[ 'month' ] = date( 'F', $date );
            $event[ 'day' ] = date( 'j', $date );
