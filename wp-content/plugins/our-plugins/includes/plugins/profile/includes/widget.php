...
add_action( 'widgets_init', function(){
	register_widget( __NAMESPACE__ . '\WP_Profile_Widget' );
});
	...
	class WP_Profile_Widget extends \WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			if( $instance['pirate_id'] !== '' ) {
				$profile = Profile_Data::get_profile($instance['pirate_id']);
			...
			if (!$hide_phone && $user->get_user()->getTelephoneNumber() != '') {
				$str .= '<div class="ecu-profile-phone ecu-widget-profile-phone"><a href="tel:' . esc_html(Profile_Data::strip_phone($user->get_user()->getTelephoneNumber())) . '"  aria-label="' . esc_html(Profile_Data::aria_phone($user->get_user()->getTelephoneNumber())). '">' . esc_html(Profile_Data::format_phone($user->get_user()->getTelephoneNumber())) . '</a></div>';
			}
			// mailstop
