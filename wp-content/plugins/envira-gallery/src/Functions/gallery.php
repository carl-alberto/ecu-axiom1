 ...
 * @return string
 */
function envira_santitize_everything( $string, $type = 'title', $for_albums = false ) {

	if ( empty( $string ) || is_array( $string ) ) {
		return;
	}

	$encoding = ( function_exists( 'mb_detect_encoding' ) && mb_detect_encoding( $string ) !== 'ASCII' ) ? mb_detect_encoding( $string ) : 'utf-8';
	...
	if ( 'envira_santitize_lightbox_caption_shortcode' === $type || 'opts-caption' === $type || 'title' === $type || 'alt' === $type || 'item-caption' === $type ) {
		$filtered_string = ( htmlentities( $string, ENT_QUOTES, $encoding ) );
	}
...
function envira_get_gallery_images( $gallery_id, $raw = false, $data = null, $return_sort_ids = false, $for_albums = false, $gallery_type = false, $cache = true ) {
	...
	if ( ! empty( $data ) && ( 'dynamic' === $data['config']['type'] || ! empty( $data['dynamic_id'] ) ) ) {
	...
	$data = apply_filters( 'envira_images_pre_data', $data, $gallery_id, false );
...
function envira_insure_random_gallery( $data, $gallery_id, $flush_cache = false ) {
	...
	$should_cache = apply_filters( 'envira_gallery_should_cache', false, $data );
	...
	// Attempt to return the transient first, otherwise generate the new query to retrieve the data.
	if ( true === $flush_cache || false === $gallery_data || false === $should_cache ) {
