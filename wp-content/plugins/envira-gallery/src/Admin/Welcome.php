...
class Welcome {
	...
	public function admin_welcome_css( $classes ) {
		...
		if ( isset( $_GET['post_type'] ) && isset( $_GET['page'] ) && 'envira' === wp_unslash( $_GET['post_type'] ) && in_array( wp_unslash( $_GET['page'] ), $upgrade_pages ) ) { // @codingStandardsIgnoreLine
	...
	public static function tab_navigation( $tab = 'whats_new' ) {
		...
		<h3 class="nav-tab-wrapper">
			...
			<a class="nav-tab
				<?php
			if ( 'envira-gallery-upgrade' === sanitize_text_field( wp_unslash( $_GET['page'] ) ) ) : // @codingStandardsIgnoreLine
					?>
				nav-tab-active<?php endif; ?>" href="
				<?php
				echo esc_url(
