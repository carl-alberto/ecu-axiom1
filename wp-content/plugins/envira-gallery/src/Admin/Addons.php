...
class Addons {
	...
	public function __construct() {
		...
		add_action( 'envira_gallery_addons_section', [ $this, 'addons_content' ] );

		// Add the addons menu item to the Plugins table.
		add_filter( 'plugin_action_links_' . plugin_basename( ENVIRA_FILE ), [ $this, 'addons_link' ] );
	...
	public function admin_menu() {
		...
		$this->hook = add_submenu_page(
			...
			apply_filters( 'envira_whitelabel', false ) ? '' : __( 'Envira Gallery ', 'envira_gallery' ) . __( 'Addons', 'envira-gallery' ),
			'<span class="envira-addons"> ' . __( 'Addons', 'envira-gallery' ) . '</span>',
		...
		if ( $this->hook ) {
			add_action( 'load-' . $this->hook, [ $this, 'maybe_refresh_addons' ] );
	...
	public function enqueue_admin_styles() {
		...
		wp_register_style( ENVIRA_SLUG . '-addons-style', plugins_url( 'assets/css/addons.css', ENVIRA_FILE ), [], ENVIRA_VERSION );
	...
	public function enqueue_admin_scripts() {
		...
		// List.js.
		wp_register_script( ENVIRA_SLUG . '-list-script', plugins_url( 'assets/js/min/list-min.js', ENVIRA_FILE ), [ 'jquery' ], ENVIRA_VERSION, true );
		wp_enqueue_script( ENVIRA_SLUG . '-list-script' );

		// Addons.
		wp_register_script( ENVIRA_SLUG . '-addons-script', plugins_url( 'assets/js/addons.js', ENVIRA_FILE ), [ 'jquery' ], ENVIRA_VERSION, true );
	...
	public function addons_page() {
		...
		<div id="addon-heading" class="subheading clearfix">
			<h2><?php apply_filters( 'envira_whitelabel', false ) ? '' : esc_html_e( 'Envira Gallery ', 'envira-gallery' ) . esc_html_e( 'Addons', 'envira-gallery' ); ?></h2>
			<form id="add-on-search">
				<span class="spinner"></span>
				<input id="add-on-searchbox" name="envira-addon-search" value="" placeholder="<?php esc_attr_e( 'Search ' . ( apply_filters( 'envira_whitelabel', false ) ? '' : 'Envira ' ) . 'Addons', 'envira-gallery' ); // @codingStandardsIgnoreLine?>" />
				<select id="envira-filter-select">
					<option value="asc"><?php esc_attr_e( 'Sort Ascending (A-Z)', 'envira-gallery' ); ?></option>
					<option value="desc"><?php esc_attr_e( 'Sort Descending (Z-A)', 'envira-gallery' ); ?></option>
				</select>
			</form>
		</div>
	...
	public function addons_content() {
		...
		if ( ! $addons ) {
			...
			<form id="envira-addons-refresh-addons-form" method="post">
				<p>
					<?php esc_html_e( 'There was an issue retrieving the addons for this site. Please click on the button below the refresh the addons data.', 'envira-gallery' ); ?>
				</p>
				<p>
					<a href="<?php echo esc_url( wp_unslash( $_SERVER['REQUEST_URI'] ) ); ?>" class="button button-primary"><?php esc_html_e( 'Refresh Addons', 'envira-gallery' ); ?></a> <?php // @codingStandardsIgnoreLine ?>
				</p>
			</form>
			<?php
			return;
		...
		<div id="envira-addons">
			...
			if ( count( $addons['licensed'] ) > 0 ) {
				...
				<div class="envira-addons-area licensed" class="envira-clear">
					<h3><?php esc_html_e( 'Available Addons', 'envira-gallery' ); ?></h3>
			...
			if ( count( $addons['unlicensed'] ) > 0 ) {
				...
				<div class="envira-addons-area unlicensed" class="envira-clear">
					...
					<p><?php echo sprintf( __( '<strong>Want even more addons?</strong> <a href="%s">Upgrade your ' . ( apply_filters( 'envira_whitelabel', false ) ? '' : 'Envira Gallery ' ) . 'account</a> and unlock the following addons.', 'envira-gallery' ), $upgrade_url ); // @codingStandardsIgnoreLine ?></p>
	...
	public function get_addons() {
		...
		$addons = get_transient( '_eg_addons' ); // @codingStandardsIgnoreLine - testing.
		...
		foreach ( (array) $addons as $i => $addon ) {
			...
			if ( 'envira-supersize' === $addon->slug || 'envira-standalone' === $addon->slug ) {
				continue;
			}
			...
			if ( (
				empty( $type ) ||
				( in_array( 'advanced', $addon->categories, true ) && 'gold' !== $type && 'platinum' !== $type && 'pro' !== $type && 'lifetime' !== $type && 'ultimate' !== $type && 'agency' !== $type ) ||
				( in_array( 'basic', $addon->categories, true ) && ( 'silver' !== $type && 'gold' !== $type && 'platinum' !== $type && 'plus' !== $type && 'pro' !== $type && 'lifetime' !== $type && 'ultimate' !== $type && 'agency' !== $type ) ) ) ||
				( 'envira-social' === $addon->slug && 'plus' !== $type && 'pro' !== $type && 'ultimate' !== $type && 'lifetime' !== $type && 'platinum' !== $type )
				// last one, see GH #3709.
			) {
				// Unlicensed.
	...
	public function get_addons_data( $key ) {
		...
		$license = new License();
		$addons  = $license->perform_remote_request( 'get-addons-data-v16', [ 'tgm-updater-key' => $key ] );
	...
	public function is_refreshing_addons() {
		...
		return isset( $_POST['envira-gallery-refresh-addons-submit'] ); // @codingStandardsIgnoreLine
	...
	public function refresh_addons_action() {
		...
		return isset( $_POST['envira-gallery-refresh-addons-submit'] ) && wp_verify_nonce( wp_unslash( $_POST['envira-gallery-refresh-addons'] ), 'envira-gallery-refresh-addons' ); // @codingStandardsIgnoreLine
	...
	public function addons_link( $links ) {
		...
		$args = [
			'post_type' => 'envira',
			'page'      => ( class_exists( 'Envira_Gallery' ) ? 'envira-gallery-addons' : 'envira-gallery-lite-addons' ),
		];

		$addons_link = sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( $args, admin_url( 'edit.php' ) ) ), __( 'Addons', 'envira-gallery' ) );
					...
					<div class="envira-addon-not-installed envira-addon-message">
						<div class="interior">
							<span class="addon-status">
									<?php esc_html_e( 'Status:', 'envira-gallery' ); ?>
									<span>
										<?php esc_html_e( 'Not Installed', 'envira-gallery' ); ?>
									</span>
								</span>
					...
					if ( is_plugin_active( $plugin_basename ) ) {
						...
						<div class="envira-addon-active envira-addon-message">
							...
							<span class="addon-status">
									<?php esc_html_e( 'Status:', 'envira-gallery' ); ?>
									<span>
										<?php esc_html_e( 'Active', 'envira-gallery' ); ?>
									</span>
								</span>
								<div class="envira-addon-action">
						...
						<div class="envira-addon-inactive envira-addon-message">
							<div class="interior">
								<span class="addon-status">
									<?php esc_html_e( 'Status:', 'envira-gallery' ); ?>
									<span>
										<?php esc_html_e( 'Inactive', 'envira-gallery' ); ?>
									</span>
								</span>
								<div class="envira-addon-action">
