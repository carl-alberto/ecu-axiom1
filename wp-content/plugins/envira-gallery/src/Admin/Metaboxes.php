...
class Metaboxes {
	...
	public function __construct() {
		...
		add_filter( 'cta_excluded_post_types', [ $this, 'envira_excluded_post_types' ], 0, 1 );
		...
		remove_filter( 'attachment_fields_to_edit', 'resmushit_image_attachment_add_status_button', null, 2 ); /* reSmush.it */
		if (
			( defined( 'DOING_AJAX' ) && ! DOING_AJAX ) ||
			( defined( 'DOING_AJAX' ) && DOING_AJAX && ( ! class_exists( 'Woo_Variation_Gallery' ) && ! class_exists( 'ACF' ) && ! isset( $GLOBALS['theiaSmartThumbnails'] ) && empty( $GLOBALS['theiaSmartThumbnails'] ) && ! isset( $GLOBALS['emr_plugin'] ) ) )
		) { /* GH 3489, 3912, 4488 */
			remove_all_filters( 'attachment_fields_to_edit', 10 );
			remove_all_filters( 'attachment_fields_to_save', 10 );
		}
		...
		if ( $this->whitelabel ) {
			add_filter( 'admin_body_class', [ $this, 'whitelabel_styles' ], 100 );
		}
		...
		add_action( 'envira_gallery_tab_mobile', [ $this, 'mobile_tab' ] );
		add_action( 'envira_gallery_tab_standalone', [ $this, 'standalone_tab' ] );
		...
		add_action( 'admin_notices', [ $this, 'notice_warnings' ] );
		add_filter( 'get_user_option_meta-box-order_envira', [ $this, 'metabox_order' ] );

		// Add Vars.
		add_action( 'admin_footer', [ $this, 'add_vars_galleries' ], 1 );

		// Mime filter to keep images only on modal upload.
		add_filter( 'post_mime_types', [ $this, 'filter_post_mime_types' ], 10, 1 );
	...
	public function filter_post_mime_types( $post_mime_types ) {
		...
		foreach ( $post_mime_types as $post_mime_type => $post_mime_type_info ) {
			if ( 'image' !== $post_mime_type ) {
				unset( $post_mime_types[ $post_mime_type ] );
			}
		}
		return $post_mime_types;
	}
	...
	public function styles() {
		...
		wp_register_style( ENVIRA_SLUG . '-metabox-style', plugins_url( 'assets/css/metabox.css', ENVIRA_FILE ), [], ENVIRA_VERSION );
	...
	public function scripts( $hook ) {
		...
		// Tabs.
		wp_register_script( ENVIRA_SLUG . '-tabs-script', plugins_url( 'assets/js/min/tabs-min.js', ENVIRA_FILE ), [ 'jquery' ], ENVIRA_VERSION, true );
		wp_enqueue_script( ENVIRA_SLUG . '-tabs-script' );

		// Clipboard.
		wp_register_script( ENVIRA_SLUG . '-clipboard-script', plugins_url( 'assets/js/min/clipboard-min.js', ENVIRA_FILE ), [ 'jquery' ], ENVIRA_VERSION, true );
		wp_enqueue_script( ENVIRA_SLUG . '-clipboard-script' );

		// Conditional Fields.
		wp_register_script( ENVIRA_SLUG . '-conditional-fields-script', envira_script( 'assets/js/min/conditional-fields-min.js' ), [ 'jquery' ], ENVIRA_VERSION, true );
		wp_enqueue_script( ENVIRA_SLUG . '-conditional-fields-script' );

		// Gallery / Album Selection.
		wp_enqueue_script( ENVIRA_SLUG . '-gallery-select-script', plugins_url( 'assets/js/gallery-select.js', ENVIRA_FILE ), [ 'jquery' ], ENVIRA_VERSION, true );
		wp_localize_script(
		...
		// Metaboxes.
		wp_register_script( ENVIRA_SLUG . '-metabox-script', envira_script( 'assets/js/min/metabox-min.js' ), [ 'jquery', 'plupload-handlers', 'quicktags', 'jquery-ui-sortable' ], ENVIRA_VERSION, true );
		...
		wp_register_script( ENVIRA_SLUG . '-media-insert-third-party', plugins_url( 'assets/js/media-insert-third-party.js', ENVIRA_FILE ), [ 'jquery' ], ENVIRA_VERSION, true );
		...
		wp_localize_script(
			...
			[
				...
				'addons'  => apply_filters( 'envira_gallery_media_insert_third_party_sources', [], $post_id ),
			]
		);
	...
	public function fix_plugin_js_conflicts() {
		...
		/* Tabs Responsive plugin from https://www.wpshopmart.com */
		if ( function_exists( 'wpsm_tabs_r_editor_popup_content_button' ) ) {
			remove_action( 'media_buttons_context', 'wpsm_tabs_r_editor_popup_content_button' );
		}
		if ( function_exists( 'wpsm_tabs_r_editor_popup_content' ) ) {
			remove_action( 'admin_footer', 'wpsm_tabs_r_editor_popup_content' );
		}
	...
	public function add_meta_boxes() {
		...
		$this->gallery_data = ( is_array( $data ) && isset( $data['id'] ) && ( ! isset( $data['config']['layout'] ) || ! $data['config']['layout'] ) ) ? envira_convert_columns_to_layouts( $data, $data['id'] ) : $data;
		...
		if ( ! in_array( $type, [ 'defaults', 'dynamic' ], true ) ) {
			add_meta_box(
				'envira-gallery',
				__( 'Envira Gallery', 'envira-gallery' ),
				[ $this, 'meta_box_gallery_callback' ],
				'envira',
				'normal',
				'high'
			);
		...
		if ( 'auto-draft' !== $post->post_status ) {
			add_meta_box(
				'envira-gallery-code',
				apply_filters( 'envira_whitelabel_name', 'Envira' ) . __( ' Code', 'envira-gallery' ),
				[ $this, 'meta_box_gallery_code_callback' ],
				'envira',
				'side',
				'default'
			);
		...
		do_action( 'envira_after_add_meta_box', $post );
	...
	public function images_tab( $post ) {
		...
		// Pass the right type to the images_display function. This is a patch at the moment. GH 3320.
		$gallery_dynamic_id = get_option( 'envira_dynamic_gallery' );
		$type               = ! empty( $gallery_dynamic_id ) && $post->ID === $gallery_dynamic_id ? 'dynamic' : envira_get_config( 'type', $data );

		// Output the tab panel for the Gallery Type.
		$this->images_display( $type, $post );
	...
	public function do_default_display( $post ) {
			...
			<nav class="envira-tab-options">
				...
				<ul class="envira-right-options">
					...
					<li class="envira-select sorting-options">
						...
						<div class="envira-tooltip"><span class="dashicons dashicons-info"></span>
							<div class="top">
									Sorting Options For Photos and Videos:
									<ul>
									<li><strong>No Sorting:</strong> Default value, sort defined by custom placement via drag-and-drop.</li>
									<li><strong>Random:</strong> Gallery randomly displays whenever cache is cleared.</li>
									<li><strong>Published Date:</strong> Sorted by WordPress's published media date.</li>
									<li><strong>Title:</strong> Sorted by title, adjustable by editing image in Envira.</li>
									<li><strong>Caption:</strong> Sorted by caption, adjustable by editing image in Envira.</li>
									<li><strong>Alt:</strong> Sorted by alt tag, adjustable by editing image in Envira.</li>
									<li><strong>Filename:</strong> Sorted by physical filename at the time of uploading into WordPress.</li>
									<li><strong>URL:</strong> Sorted by URL field, adjustable by editing image in Envira.</li>
									<?php do_action( 'envira_get_sorting_options_tooltip' ); ?>
									</ul>
							</div>
						</div>
					</li>
		...
		if ( '' === envira_get_license_key_type() || 'basic' === envira_get_license_key_type() ) {
			...
			$this->notices->display_inline_notice(
				...
				<ul>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/demo/albums-demo', 'basicadminpagegallery', 'albums' ) . '">Create Albums & Cover Photos For Your Galleries</a></li>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/demo/video-gallery-demo', 'basicadminpagegallery', 'addvideos' ) . '">Add Videos To Your Galleries from Instagram, Youtube, Twitch and more</a></li>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/demo/envira-gallery-theme-demo', 'basicadminpagegallery', 'gallerythemes' ) . '">Add More Themes/Layouts to Your Galleries</a></li>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/demo/instagram-gallery-demo', 'basicadminpagegallery', 'buildinstagram' ) . '">Build Galleries from Instagram</a></li>
				</ul>
				<ul>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/demo/downloads-gallery-demo', 'basicadminpagegallery', 'buildinstagram' ) . '">Give Users the Ability to Download Your Images</a></li>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/demo/gallery-pagination-demo', 'basicadminpagegallery', 'pagination' ) . '">Speed Up Your Page Loading Times with Pagination</a></li>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/addons/woocommerce-addon', 'basicadminpagegallery', 'sellPhotos' ) . '">Sell Your Photos in WordPress with WooCommerce</a></li>
				     <li>Dedicated customer support and so much more!</li>
				</ul>
					</div>', 'envira-gallery' ),
					'warning',
					__( 'Click here to Upgrade', 'envira-gallery' ),
					envira_get_upgrade_link( $upgrade_link, 'adminpagegallery', 'clickheretoupgradebutton' ),
					false,
					false
			);
		} else if ( 'plus' === envira_get_license_key_type() ) {
			// Output an upgrade notice.
			$this->notices->display_inline_notice(
				'envira_gallery_images_tab',
							__( 'Want to make your gallery workflow and presentation even better?', 'envira-gallery' ),
				__( '<p>By upgrading to Envira Pro, you can get access to numerous other features, including:</p><div class="two-column-list">
				<ul>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/demo/albums-demo', 'plusadminpagegallery', 'albums' ) . '">Create Albums & Cover Photos For Your Galleries</a></li>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/demo/video-gallery-demo', 'plusadminpagegallery', 'addvideos' ) . '">Add Videos To Your Galleries from Instagram, Youtube, Twitch and more</a></li>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/demo/downloads-gallery-demo', 'plusadminpagegallery', 'buildinstagram' ) . '">Give Users the Ability to Download Your Images</a></li>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/addons/woocommerce-addon', 'plusadminpagegallery', 'sellPhotos' ) . '">Sell Your Photos in WordPress with WooCommerce</a></li>

				</ul>
				<ul>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/demo/protection-demo', 'plusadminpagegallery', 'protection' ) . '">Protect Your Images from Online Theft</a></li>
					<li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/how-to-upload-photos-directly-from-lightroom-to-wordpress', 'plusadminpagegallery', 'lightroom' ) . '">Upload Your Photos Directly From Adobe Lightroom to WordPress</a></li>
				    <li>Dedicated customer support and so much more!</li>
					</ul>
					</div>', 'envira-gallery' ),
					'warning',
	...
	public function get_gallery_item( $id, $item, $post_id = 0 ) {
		...
		$item['alt']        = isset( $item['alt'] ) ? str_replace( '&quot;', '\"', $item['alt'] ) : false;
		$item['_thumbnail'] = ( ! empty( $thumbnail[0] ) ) ? $thumbnail[0] : $item['src']; // Never saved against the gallery item, just used for the thumbnail output in the Edit Gallery screen.

		$item['status'] = isset( $item['status'] ) ? $item['status'] : false;
		$item['title']  = isset( $item['title'] ) ? $item['title'] : false;
		$item['link']   = isset( $item['link'] ) ? $item['link'] : false;
	...
	public function config_tab( $post ) {
		...
		if ( isset( $this->gallery_data['id'] ) ) {
			$gallery_data = ( ! $this->gallery_data['config']['layout'] ) ? envira_convert_columns_to_layouts( $this->gallery_data, $this->gallery_data['id'] ) : envira_override_layout_settings( $this->gallery_data );
		} else {
			$gallery_data = false;
		}
		...
		<div id="envira-config">
			...
			<table class="form-table" style="margin-bottom: 0;">
				<tbody>
				<tr id="envira-config-layouts-box">
						<th scope="row">
							<label for="envira-config-layouts"><?php esc_html_e( 'Gallery Layout', 'envira-gallery' ); ?></label>
						</th>
						<td>
							<select id="envira-config-layouts" class="image-picker show-labels show-html" name="_envira_gallery[layout]">
								<?php foreach ( (array) envira_get_layouts() as $value => $data ) : ?>
									<option data-img-src="<?php echo esc_html( $data['thumbnail'] ); ?>" value="<?php echo esc_html( $value ); ?>"<?php selected( $value, envira_get_config( 'layout', $gallery_data ) ); ?>><?php echo esc_html( $data['name'] ); ?></option>
								<?php endforeach; ?>
							</select>
							<p class="description"><?php esc_html_e( 'Determines the Layout for the gallery.', 'envira-gallery' ); ?></p>
						</td>
					</tr>
					<tr id="envira-config-columns-box">
						...
						<td>
							...
							<p class="description"><?php esc_html_e( 'Determines the number of columns in the gallery.', 'envira-gallery' ); ?></p>
						</td>
					</tr>
				...
				<table class="form-table" style="margin-bottom: 0;">
					<tbody>
						...
						<tr id="envira-config-additional-copy-box-automatic">
							...
							<td>
								<select id="envira-config-gallery-automatic-title-caption" name="_envira_gallery[gallery_automatic_title_caption]">
									<?php foreach ( (array) envira_get_additional_copy_options() as $option_value => $option_name ) : ?>
										<option value="<?php echo esc_html( $option_value ); ?>" <?php selected( $option_value, envira_get_config( 'gallery_automatic_title_caption', $gallery_data ) ); ?>><?php echo esc_html( $option_name ); ?></option>
									<?php endforeach; ?>
								</select><br>
							</td>
						</tr>
						...
						<tr id="envira-config-additional-copy-box">
							...
							<td>
								<select id="envira-config-gallery-column-title-caption" name="_envira_gallery[gallery_column_title_caption]">
									<?php foreach ( (array) envira_get_additional_copy_options() as $option_value => $option_name ) : ?>
										<option value="<?php echo esc_html( $option_value ); ?>" <?php selected( $option_value, envira_get_config( 'gallery_column_title_caption', $gallery_data ) ); ?>><?php echo esc_html( $option_name ); ?></option>
									<?php endforeach; ?>
								</select><br>
							</td>
						</tr>
						...
						<tr id="envira-config-square-size-box">
						<th scope="row">
							<label for="envira-config-crop-width"><?php esc_html_e( 'Square Dimensions', 'envira-gallery' ); ?></label>
						</th>
						<td>
							<input id="envira-config-crop-width" type="number" name="_envira_gallery[square_size]" value="<?php echo esc_html( envira_get_config( 'square_size', $gallery_data, envira_get_config_default( 'square_size' ) ) ); ?>" /> <?php esc_html_e( 'size (px)', 'envira-gallery' ); ?> &#215;
							<p class="description"><?php esc_html_e( 'Size of the square the image will be cropped too.', 'envira-gallery' ); ?></p>
						</td>
						</tr>
						<!-- Dimensions -->
						...
						<td>
							...
							<p class="description"><?php esc_html_e( 'Define the maximum image size for the Gallery view. Default will use the below Image Dimensions.', 'envira-gallery' ); ?></p>
						</td>
					</tr>
			...
			<table class="form-table">
				<tbody>
					...
					<tr id="envira-config-title-caption-automatic-mobile">
							...
							<td>
								<select id="envira-config-gallery-column-title-caption" name="_envira_gallery[gallery_automatic_title_caption_mobile]">
									<?php foreach ( (array) envira_get_additional_copy_options() as $option_value => $option_name ) : ?>
										<option value="<?php echo esc_html( $option_value ); ?>" <?php selected( $option_value, envira_get_config( 'gallery_automatic_title_caption_mobile', $gallery_data ) ); ?>><?php echo esc_html( $option_name ); ?></option>
									<?php endforeach; ?>
								</select><br>
							</td>
						</tr>
						<tr id="envira-config-title-caption-column-mobile">
							...
							<td>
								<select id="envira-config-gallery-column-title-caption" name="_envira_gallery[gallery_column_title_caption_mobile]">
									<?php foreach ( (array) envira_get_additional_copy_options() as $option_value => $option_name ) : ?>
										<option value="<?php echo esc_html( $option_value ); ?>" <?php selected( $option_value, envira_get_config( 'gallery_column_title_caption_mobile', $gallery_data ) ); ?>><?php echo esc_html( $option_name ); ?></option>
									<?php endforeach; ?>
								</select><br>
							</td>
						</tr>
			...
			<table class="form-table">
				<tbody>
					<tr id="envira-config-standalone-box">
							...
							<td>
								<?php if ( function_exists( 'wp_is_block_theme' ) && function_exists( 'get_block_templates' ) && wp_is_block_theme() ) { ?>
									<?php

										// the current theme is YES a block theme.
										$block_theme_templates = get_block_templates();
									...
									if ( ! empty( $block_theme_templates ) ) :

										?>
									<select id="envira-config-standalone-template" name="<?php echo esc_html( $key ); ?>[standalone_template]">
										<option value="">(Default)</option>
										<?php foreach ( (array) $block_theme_templates as $index => $template_post ) : ?>

										<option value="<?php echo esc_html( $template_post->slug ); ?>"<?php selected( $template_post->slug, envira_get_config( 'standalone_template', $gallery_data ) ); ?>><?php echo esc_html( $template_post->title ); ?></option>
										<?php endforeach; ?>
									</select>
									<p class="description"><?php esc_html_e( 'You seem to have a block theme active. By default block themes use the single template.', 'envira-gallery' ); ?> <a href="<?php echo admin_url('site-editor.php?postType=wp_template'); ?>"><?php esc_html_e( 'View your templates here' ); ?></a></p> <?php // @codingStandardsIgnoreLine ?>
			...
			<table class="form-table">
				<tbody>
					...
					<?php
						...
						$galleries_for_copy = ( function_exists( 'envira_get_galleries_title_id' ) ) ? envira_get_galleries_title_id( false ) : envira_get_galleries( false ); // do not skip empty galleries, but respect cache.

					?>

					<tr id="envira-config-copy-images">
						<th scope="row">
							<label for="envira-config-import-gallery"><?php esc_html_e( 'Copy Images', 'envira-gallery' ); ?></label>
						</th>
						<td>
							<?php

							if ( empty( $galleries_for_copy ) ) {

								?>
							<p><strong>You currently have no available galleries to copy images to.</strong></p>
							<?php } else { ?>
							<form id="envira-config-copy-images" method="post">
								<input type="hidden" name="envira_copy_images" value="1" />
								<select name="envira_copy_target_gallery" style="max-width: 75%; width: 75%;">
									<?php
									foreach ( $galleries_for_copy as $gallery_to_copy_id => $gallery_to_copy_title ) {
										?>
										<?php
										if ( empty( $gallery_to_copy_title ) ) {
											continue;
										} else {
											$gallery_to_copy_title = strlen( $gallery_to_copy_title ) > 70 ? substr( $gallery_to_copy_title, 0, 68 ) . '...' : $gallery_to_copy_title;
										}
										?>
									<option value="<?php echo intval( $gallery_to_copy_id ); ?>"><?php echo esc_html( $gallery_to_copy_title ); ?></option>
									<?php } ?>
								</select>
								<?php wp_nonce_field( 'envira-gallery-copy-images', 'envira-gallery-copy-images' ); ?>
								<?php submit_button( __( 'Copy', 'envira-gallery' ), 'button button-primary', 'envira-gallery-export-submit', false ); ?>
							</form>
							<?php } ?>
							<p class="description">
								<?php
								// Translators: %1$s.
								echo ( sprintf( __( 'Copy images to another existing gallery (will not copy images already existing in target gallery). If your published gallery is not showing in this list try <a href="%1$s">clearing transients</a>.', 'envira-gallery' ), admin_url('edit.php?post_type=envira&page=envira-gallery-tools') ) ); // @codingStandardsIgnoreLine
								?>
							</p>
						</td>
					</tr>
	...
	public function save_meta_boxes( $post_id, $post ) {
		...
		$settings['config']['layout']             = isset( $_POST['_envira_gallery']['layout'] ) ? preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['layout'] ) ) ) : envira_get_config_default( 'layout' );
		...
		$settings['config']['square_size']        = isset( $_POST['_envira_gallery']['square_size'] ) ? absint( $_POST['_envira_gallery']['square_size'] ) > 0 ? absint( $_POST['_envira_gallery']['square_size'] ) : envira_get_config_default( 'square_size' ) : envira_get_config_default( 'square_size' );
		...
		$settings['config']['gallery_column_title_caption']        = isset( $_POST['_envira_gallery']['gallery_column_title_caption'] ) ? preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['gallery_column_title_caption'] ) ) ) : envira_get_config_default( 'gallery_column_title_caption' );
		$settings['config']['gallery_column_title_caption_mobile'] = isset( $_POST['_envira_gallery']['gallery_column_title_caption_mobile'] ) ? preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['gallery_column_title_caption_mobile'] ) ) ) : envira_get_config_default( 'gallery_column_title_caption_mobile' );
		...
		$settings['config']['gallery_automatic_title_caption']        = isset( $_POST['_envira_gallery']['gallery_automatic_title_caption'] ) ? preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['gallery_automatic_title_caption'] ) ) ) : envira_get_config_default( 'gallery_automatic_title_caption' );
		$settings['config']['gallery_automatic_title_caption_mobile'] = isset( $_POST['_envira_gallery']['gallery_automatic_title_caption_mobile'] ) ? preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['gallery_automatic_title_caption_mobile'] ) ) ) : envira_get_config_default( 'gallery_automatic_title_caption_mobile' );
		...
		if ( 0 === $settings['config']['thumbnails_toggle'] ) {
			$settings['config']['thumbnails_hide'] = 0;
		}
		...
		// Override layout settings, as 1 column layout does not exist in the dropdown and might get resaved, etc.
		$settings = envira_override_layout_settings( $settings );
		...
		// if albums addon is activated, make sure name is updated in all the albums.
		// ideally (?) there would be a metadata value for gallery ids in albums similar to _eg_in_gallery so we wouldn't need to go through all albums.

		if ( class_exists( 'Envira_Albums' ) && function_exists( '_envira_get_albums' ) ) :

			$albums = _envira_get_albums( true ); // skip empty.
			if ( ! empty( $albums ) ) :
				foreach ( $albums as $album ) {
					if ( isset( $_POST['post_title'] ) && isset( $album['id'] ) && isset( $album['galleries'][ $post_id ] ) && ! empty( $album['galleries'][ $post_id ] ) ) {
						$album['galleries'][ $post_id ]['title'] = esc_html( $_POST['post_title'] ); // @codingStandardsIgnoreLine
						update_post_meta( $album['id'], '_eg_album_data', $album );
						envira_flush_album_caches( $album['id'] );
					}
				}
			endif;
		...
		$current_slug = ! empty( $settings['config']['slug'] ) ? $settings['config']['slug'] : false;
		$slug_to_save = false;

		if ( false !== $current_slug ) {
			// if it's changed, update it.
			$permalink_slug = ( ! empty( $_POST['post-name'] ) ) ? sanitize_title( wp_unslash( $_POST['post-name'] ) ) : sanitize_title( $post->post_name );
			$manual_slug    = ( ! empty( $_POST['_envira_gallery']['slug'] ) ) ? sanitize_title( wp_unslash( $_POST['_envira_gallery']['slug'] ) ) : sanitize_title( $post->post_name );
			if ( $permalink_slug !== $current_slug && false === strpos( strtolower( $permalink_slug ), 'auto-draft' ) ) {
				$slug_to_save = $permalink_slug;
			} else {
				$slug_to_save = $manual_slug;
			}
		} else {
			// if the current slug is... nothing.
			$slug_to_save = ( ! empty( $_POST['post-name'] ) ) ? sanitize_title( wp_unslash( $_POST['post-name'] ) ) : sanitize_title( $post->post_name );
		}

		$settings['config']['slug'] = ( false !== $slug_to_save ) ? $slug_to_save : false;

		// We need to add metadata if the config slug doesn't match.
		if ( ! empty( $slug_to_save ) && $slug_to_save !== $post->post_name ) {

			$existing_page = get_page_by_path( $slug_to_save, ARRAY_A, [ 'envira', 'envira_album', 'post', 'page' ] );
			...
			if ( $existing_page ) {
				...
				add_filter( 'redirect_post_location', [ $this, 'add_notice_slug_exists' ], 99 );
		...
		if ( 'square' === $settings['config']['layout'] ) {

			$settings['config']['crop']        = 1;
			$settings['config']['crop_width']  = isset( $_POST['_envira_gallery']['square_size'] ) ? absint( $_POST['_envira_gallery']['square_size'] ) > 0 ? absint( $_POST['_envira_gallery']['square_size'] ) : envira_get_config_default( 'square_size' ) : envira_get_config_default( 'square_size' );
			$settings['config']['crop_height'] = isset( $_POST['_envira_gallery']['square_size'] ) ? absint( $_POST['_envira_gallery']['square_size'] ) > 0 ? absint( $_POST['_envira_gallery']['square_size'] ) : envira_get_config_default( 'square_size' ) : envira_get_config_default( 'square_size' );
		}
		// This is forced to make sure any left over isotope checks are overrode.
		if ( 'mason' === $settings['config']['layout'] ) {
			$settings['config']['isotope'] = 1;
		}
	...
	public function add_notice_slug_exists( $location ) {
		...
		return add_query_arg( [ 'envira_slug_exists' => $this->duplicate_post_id ], $location );
	 ...
	 * @since 1.8.7
	 */
	public function add_vars_galleries() {

		echo "
			<script type=\"text/javascript\">
				    window.envira_back_to_gallery_link = '" . esc_url( admin_url( 'edit.php?post_type=envira' ) ) . "',
				    window.envira_back_to_gallery_text = '" . esc_html( __( 'Galleries', 'envira-gallery' ) ) . "';
			</script>
		";
