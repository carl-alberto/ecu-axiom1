...
class Support {
	...
	public function __construct() {
		...
		add_action( 'admin_init', array( $this, 'tools' ), 10 );
		...
		add_action( 'envira_support_notices', array( $this, 'tools_repair_gallery' ), 10 );
	...
	public function envira_edit_gallery_settings() {
		...
		foreach ( $oringial_settings['config'] as $oringial_setting => $oringial_value ) {
			...
			if ( in_array( 'value-' . $oringial_setting, $updated_settings, true ) && $updated_settings[ 'value-' . $oringial_setting ] !== $oringial_value ) {
	...
	public function tools_repair_gallery() {
		...
		if ( $_POST['action'] !== 'tools-repair-gallery' ) {// @codingStandardsIgnoreLine
			return;
		}
		...
		<div class="notice notice-warning">
			...
			if ( $removed > 0 ) {
				...
				echo '<p>Removed a total of <strong>' . esc_html( $removed ) . '</strong> items for the <strong>' . esc_html( $gallery_title ) . '</strong> gallery.</p></div>';
	...
	public function clear_all_envira_options() {
		...
		if ( 'delete-options' === $_POST['action'] ) { // @codingStandardsIgnoreLine
			...
			// echo $envira_display_welcome; exit;
		}

		if ( 'toggle-debug' === $_POST['action'] ) {
	 ...
	 */
	 public function tools() {
		...
		if ( empty( $_GET['page'] ) || 'envira-gallery-support-tools' !== $_GET['page'] || ( 'envira-gallery-support-tools' === $_GET['page'] && empty( $_POST ) ) ) { // @codingStandardsIgnoreLine
			return;
		}

		if ( isset( $_POST['action'] ) && 'tool_settings' === $_POST['action'] ) {  // @codingStandardsIgnoreLine

			if ( isset( $_POST['support_show_ssl_filter'] ) && intval( $_POST['support_show_ssl_filter'] ) === 1 ) {  // @codingStandardsIgnoreLine
				update_option( 'eg_support_ssl_filters', 1, null, 'no' );
			} else {
				delete_option( 'eg_support_ssl_filters' );
			}

			do_action( 'envira_support_save_tools' );
		...
		<div class="envira-welcome-wrap envira-welcome">
				...
				<div class="wrap">
					...
					<div class="card">
						...
						<ul>
							<li>Current Version In Options: <?php echo $envira_version; ?></li>
							<li>Display Welcome: <?php echo $envira_display_welcome; ?></li>
							<li>Version Test: <?php echo $envira_version_test; ?></li>
						</ul>
		...
		<div class="envira-welcome-wrap envira-welcome">
				...
				<div class="wrap">
					...
					<div class="card">
						...
						<p><label>Select Gallery:</label>
							...
							<?php } ?>
								...
								<input type="submit" class="button button-primary" value="<?php _e( 'Repair Gallery', 'envira-gallery' ); ?>" />
							</form>
					</div>
					...
					<div class="card">
						...
						$filter_option = get_option( 'eg_support_ssl_filters' );
						$filter_active = ! empty( $filter_option ) && absint( $filter_option ) === 1 ? true : false;

						?>
						<h2 class="title">Apply SSL (http/s) Filters</h2>
						<p class="subtitle"><small>Applies 'envira_fix_ssl_images' fix without having to install plugin or add to functions.php (see Github Ticket #3496).</small></p>
						<form action="" method="post">
							<input type="hidden" name="action" value="tool_settings" />
							...
							<tbody>
								...
								<tr id="envira-media-position-box">
									<th scope="row">
										<label for="envira-media-position">Apply filters?</label>
									</th>
									<td>
										<select name="support_show_ssl_filter" id="support_show_ssl_filter" name="support_show_ssl_filter">
											<option value="0"
											<?php
											if ( ! $filter_active ) {
												?>
							...
							<input type="submit" class="button button-primary" value="<?php _e( 'Toggle Filter', 'envira-gallery' ); ?>" />
						</form>
					</div>
	...
	/**
	 * Hook into actions to resolve ssl images
	 *
	 * @since 1.8.8
	 */
	public function fix_ssl_images() {

		if ( is_admin() ) {
			return;
		}

		add_filter( 'wp_get_attachment_url', array( $this, 'envira_fix_ssl_images' ), 10, 1 );
		add_filter( 'envira_image_src', array( $this, 'envira_fix_ssl_images' ), 10, 1 );
		add_filter( 'envira_output_src', array( $this, 'envira_fix_ssl_images' ), 10, 1 );
		add_filter( 'envira_albums_image_src', array( $this, 'envira_fix_ssl_images' ), 10, 1 );
	 ...
	 * @since 1.8.8
	 */
	public function envira_fix_ssl_images( $url ) {
