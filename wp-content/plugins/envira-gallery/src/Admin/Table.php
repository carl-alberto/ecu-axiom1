...
class Table {
	...
	public function __construct() {
		...
		add_action( 'pre_get_posts', [ $this, 'pre_get_posts' ], 51 ); // upped to 51 because of Event Calendar plugin.

		// Expand search with IDs in addition to WordPress default of post/gallery titles.
		add_action( 'posts_where', [ $this, 'enable_search_by_gallery_id' ], 10 );
	...
	public function pre_get_posts( $query ) {
		...
		if ( is_admin() && 'edit.php' === $GLOBALS['pagenow']
			...
			add_filter( 'option_sticky_posts', [ $this, 'custom_stickies' ] );
	...
	public function scripts() {
		...
		wp_register_script( ENVIRA_SLUG . '-clipboard-script', plugins_url( 'assets/js/min/clipboard-min.js', ENVIRA_FILE ), [ 'jquery' ], ENVIRA_VERSION, false );
		...
		wp_register_script( ENVIRA_SLUG . '-gallery-select-script', plugins_url( 'assets/js/gallery-select.js', ENVIRA_FILE ), [ 'jquery' ], ENVIRA_VERSION, true );
		wp_localize_script(
		...
		wp_register_script( ENVIRA_SLUG . '-table-script', plugins_url( 'assets/js/min/table-min.js', ENVIRA_FILE ), [ 'jquery' ], ENVIRA_VERSION, false );
