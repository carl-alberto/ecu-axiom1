<?php
	...
	class Dashboard extends Abstract_Ecu_Shortcode {
		...
		public function initialize(){
			...
			if ( is_admin() ) {
				add_editor_style( plugins_url('/our-plugins/includes/plugins/dashboard/css/style.css') );
			}
			...
			if($dashboard = get_field('ui_dashboard', $attrs['dashboard_id'])){
				...
				foreach($dashboard as $col){
					...
					if($col['link_element']){
						...
						$output .= "<a class='no-text-decoration' href='{$link}' ".($col['external_link'] ? 'target="_blank"' : '').">";
					}
					if($col['icon'] || $col['image']){
						$output .= '<div class="dashboard_icon">';
						if($col['custom_image']){
							$output .= '<div class="icon-wrap '.$col['background'].' '. $icon_size.' dash-no-bg" style="background-image:url('.$col['image'].');" >';
						} else {
							$output .= '<div class="icon-wrap '.$col['background'].' '. $icon_size.' " >';
						}
						if(!$col['custom_image']){
