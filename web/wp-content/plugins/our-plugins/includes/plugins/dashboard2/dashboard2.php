<?php
	...
	class Dashboard2 extends Abstract_Ecu_Shortcode {
		...
		public function initialize(){
			...
			if ( is_admin() ) {
				add_editor_style( plugins_url('/our-plugins/includes/plugins/dashboard2/css/style.css') );
			}
		...
		public function enqueue_scripts() {
			wp_register_style( 'ecu-shortcode-dashboard2', plugins_url('/our-plugins/includes/plugins/dashboard2/css/style.css') );
		}
		...
		public function render_polaroid($item) {
			...
			$this->output .= '<div class="polaroid-wrapper item '.$item['color'].'">';
				...
				$this->output .= '</div>';
				if ($item['icon_link']) {
					$this->output .= '<a class="no-text-decoration" href="'.$item['icon_link']['url'].'" target="'.$item['icon_link']['target'].'">';
				}
		...
		public function render_horizontal_icon($item) {
			...
			$this->output .= '<div class="icon-wrapper item '.$item['color'].' link-wrap ">';
		...
		public function render_icon_with_content($item) {
			$this->output .= '<div class="icon-content-wrapper item '.$item['color'].'">';
				if ($item['icon_link']) {
					$this->output .= '<a class="no-text-decoration" href="'.$item['icon_link']['url'].'" target="'.$item['icon_link']['target'].'">';
				}
		...
		public function render_icon_with_links($item) {
			$this->output .= '<div class="icon-links-wrapper item '.$item['color'].'">';
				if ($item['icon_link']) {
					$this->output .= '<a class="no-text-decoration" href="'.$item['icon_link']['url'].'" target="'.$item['icon_link']['target'].'">';
				}
				$this->output .= '<div class="link-wrap">';
				...
				$this->output .= '<div class="dash-content col-12">';
					$this->output .= '<ul class="link-list">';
						foreach ($item['links'] as $link) {
							$this->output .= '<li>';
								$this->output .= '<a class="no-text-decoration" href="'.$link['url']['url'].'" target="'.$link['url']['target'].'">'.$link['url']['title'].'</a>';
							$this->output .= '</li>';
						}
					$this->output .= '</ul>';
				$this->output .= '</div>';
