<?php
	...
	class Link_Farm extends Abstract_Ecu_Shortcode {
		...
		public function initialize(){
			...
			if ( is_admin() ) {
				add_editor_style( plugins_url('/ecu-plugins/includes/plugins/link-farm/css/style.css'));
			}
		...
		public function enqueue_scripts() {
			wp_register_style( 'ecu-shortcode-link-farm', plugins_url('/ecu-plugins/includes/plugins/link-farm/css/style.css' ));
		}
				...
				foreach($farm as $column){
					...
					$output .= '<div class="ui_link_farm_links">
						...
						foreach($column['links'] as $link){
							...
							$output .= '<li><a href="'.esc_url($page_link).'"';
