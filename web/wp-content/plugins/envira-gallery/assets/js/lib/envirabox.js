(function (window, document, $, undefined) {
	...
	$.extend(EnviraBox.prototype, {
		...
		getFitPos: function (slide) {
			...
			if (
				(isChrome && matchMedia(query).matches) ||
			...
			canvasWidth =
				Math.abs(parseInt(self.$refs.stage.width(), 10) -
				(margin[1] + margin[3]));
			canvasHeight =
				Math.abs(parseInt(self.$refs.stage.height(), 10) -
				(margin[0] + margin[2]));
	...
	$.envirabox = {
		...
		getInstance: function (command) {
			const selector = '.envirabox-container:not(".envirabox-is-closing"):first';
			let instance = $(selector).data('envirabox');
			let args = Array.prototype.slice.call(arguments, 1);
