...
class Import {
	...
	public function __construct() {
		...
		add_action( 'init', [ $this, 'import_gallery' ] );
		add_action( 'admin_notices', [ $this, 'notices' ] );

		// WordPress XML Import.
		add_action( 'import_post_meta', [ $this, 'change_xml_import' ], 10, 3 );
	...
	public function import_gallery() {
		...
		// redirect to avoid double imports?
		wp_safe_redirect( admin_url( 'post.php?post=' . $post_id . '&action=edit&envira-gallery-imported=1#!envira-tab-misc' ) );
		exit;
	...
	public function import_remote_image( $src, $data, $item, $post_id, $id = 0, $stream_only = false ) {
		...
		// Prepare variables.
		$stream   = wp_remote_get( $src, [ 'timeout' => 60 ] );
		...
		if ( is_wp_error( $stream ) ) {
			...
			if ( ! empty( $mirror['error'] ) ) {
				...
				if ( ! is_array( $data['in_gallery'] ) ) {
					$data['in_gallery'] = []; // @codingStandardsIgnoreLine
				}
