...
class License {
	...
	public function verify_key() {
		...
		$verify = $this->perform_remote_request( 'verify-key', array( 'tgm-updater-key' => wp_unslash( $_POST['envira-license-key'] ) ) ); // @codingStandardsIgnoreLine
		...
		update_option( 'envira_can_beta', isset( $verify->beta ) ? $verify->beta : false );
	...
	public function is_verifying_key() {
		...
		return isset( $_POST['envira-license-key'] ) && isset( $_POST['envira-gallery-verify-submit'] ); // @codingStandardsIgnoreLine
	...
	public function verify_key_action() {
		...
		return isset( $_POST['envira-gallery-verify-submit'] ) && isset( $_POST['envira-gallery-key-nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['envira-gallery-key-nonce'] ) ), 'envira-gallery-key-nonce' );
	...
	public function validate_key( $forced = false ) {
		...
		$validate = $this->perform_remote_request( 'validate-key', [ 'tgm-updater-key' => $this->key ] );
		...
		$option['is_disabled'] = false;
		$option['is_invalid']  = false;
		update_option( 'envira_can_beta', isset( $validate->beta ) ? $validate->beta : false );
		update_option( 'envira_gallery', $option );
	...
	public function deactivate_key() {
		...
		$deactivate = $this->perform_remote_request( 'deactivate-key', array( 'tgm-updater-key' => $_POST['envira-license-key'] ) ); // @codingStandardsIgnoreLine
	...
	public function is_deactivating_key() {
		...
		return isset( $_POST['envira-license-key'] ) && isset( $_POST['envira-gallery-deactivate-submit'] ); // @codingStandardsIgnoreLine
	...
	public function deactivate_key_action() {
		...
		return isset( $_POST['envira-gallery-deactivate-submit'] ) && isset( $_POST['envira-gallery-key-nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['envira-gallery-key-nonce'] ) ), 'envira-gallery-key-nonce' );
	...
	public function is_refreshing_key() {
		...
		return isset( $_POST['envira-license-key'] ) && isset( $_POST['envira-gallery-refresh-submit'] ); // @codingStandardsIgnoreLine
	...
	public function refresh_key_action() {
		...
		return isset( $_POST['envira-gallery-refresh-submit'] ) && isset( $_POST['envira-gallery-key-nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['envira-gallery-key-nonce'] ) ), 'envira-gallery-key-nonce' );
	...
	public function notices() {
		...
		if ( ! $key ) :
			...
			$message = sprintf(
				/* translators: %s: URL */
				__( '<strong>Envira Gallery</strong>: In order to create new galleries, access addons, and receive automatic updates for Envira Gallery, <a href="%s">please enter a valid license key here.</a>', 'envira-gallery' ),
				esc_url(
					add_query_arg(
						[
							'post_type' => 'envira',
		...
		if ( ! empty( $this->success ) ) :
			?>
		<div class="updated">
			<p><?php echo implode( '<br>', $this->success ); // @codingStandardsIgnoreLine ?></p>
		</div>
			<?php
		endif;
