...
class Notices {
	 ...
	 * @param bool    $updated            If updated class is added WordPress moves it into the notices area in the admin.
	 */
	public function display_inline_notice( $notice, $title, $message, $type = 'success', $button_text = '', $button_url = '', $is_dismissible = true, $seconds = false, $updated = true ) {

		$check_option = 0 < $seconds ? true : false;

		// Check if the notice is dismissible, and if so has been dismissed.
		if ( $is_dismissible && $this->is_dismissed( $notice, $check_option ) ) {
		...
		?>
		<div class="<?php $updated ? 'updated ' : ''; ?>envira-notice <?php echo sanitize_html_class( $type ); ?> <?php echo sanitize_html_class( ( $is_dismissible ? ' is-dismissible' : '' ) ); ?>" data-seconds="<?php echo esc_attr( $seconds ); ?>" data-notice="<?php echo esc_attr( $notice ); ?>">
			<?php
			// Title.
			if ( ! empty( $title ) ) {
				?>
