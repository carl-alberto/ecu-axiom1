<?php
	...
	/*
	Plugin Name: WP Localist Calendar
	...
	*/
	...
	add_action('wp_enqueue_scripts', 'WPLocalist\enqueue_global_styles', 15, 0);
	...
	if( is_admin() ) {
		...
		add_action('init', 'WPLocalist\enqueue_admin_styles', 15, 0);

		// Register the uninstall function.
	 	register_uninstall_hook( plugin_dir_path( __FILE__ ), 'WPLocalist\uninstall_plugin' );


		// Register the menu for the plugin
		if(is_multisite()) {
			add_action( 'network_admin_menu', function(){
				add_submenu_page('settings.php', 'Localist Calendar', 'Localist Calendar', 'manage_options', 'wp-localist-network', 'WPLocalist\settings_form'  );
			},10,0);
		} else {
			add_action('admin_menu', function(){
				add_submenu_page( 'options-general.php', 'Localist Calendar', 'Localist Calendar', 'manage_options', 'wp-localist-admin', 'WPLocalist\settings_form' );
			},10,0);
		}
	}