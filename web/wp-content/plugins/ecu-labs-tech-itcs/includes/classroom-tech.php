...
namespace ECU\CLASSROOMTECH;


// define shortcode
add_shortcode('ecu_classroom_tech_search', 'ECU\CLASSROOMTECH\render_table');
add_shortcode('ecu_classroom_tech_legend', 'ECU\CLASSROOMTECH\render_legend');

// load specific scripts and styles
add_action( 'wp_enqueue_scripts', 'ECU\CLASSROOMTECH\load_plugin_assets',10,0);
...
function load_plugin_assets() {
	...
	if( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'ecu_classroom_tech_search') ) {
        ...
        \ECU\GLOBALASSETS\register_datatables_buttons();
...
function adding_scripts() {
    wp_register_script('ecu_classroomtech_js', plugins_url( '/ecu-labs-tech-itcs/templates/classroom-tech/classroom-tech.js' ) , array('jquery'), '1.1', true);
    wp_enqueue_script('ecu_classroomtech_js');
    // add styles
    adding_styles();
}
function adding_styles() {
    wp_register_style('ecu_classroomtech_css', plugins_url( '/ecu-labs-tech-itcs/templates/classroom-tech/classroom-tech.css' ) );
