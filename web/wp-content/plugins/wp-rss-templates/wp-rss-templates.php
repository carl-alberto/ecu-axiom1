...
// This is needed since v0.2 had a different folder name, "wp-rss-templates-0.2", which allowed updates to result
// in 2 different copies of the addon to be installed and activated.
if (defined('WPRSS_TEMPLATES') && strpos(WPRSS_TEMPLATES, '0.2') !== false) :
    if (!function_exists('is_plugin_active')) {
        require_once ABSPATH . 'wp-admin/includes/plugin.php';
    }

    $basename = plugin_basename(WPRSS_TEMPLATES);
    deactivate_plugins($basename);
    delete_plugins([$basename]);
...
if (!defined('WPRSS_TMP_SL_STORE_URL')) {
    define('WPRSS_TMP_SL_STORE_URL', 'http://wprssaggregator.com/edd-sl-api/');
}
